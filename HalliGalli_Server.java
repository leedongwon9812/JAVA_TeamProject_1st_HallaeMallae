...
public class HalliGalli_Server
{
	...
	public void startServer()
	{
		try
		{
			...
			System.out.println("호스트서버 IP주소 : " + hostAddress);
			while (true)
			{
				// 클라이언트와 연결된 스레드를 얻는다.
	...
	*/
				...
				while ((msg = reader.readLine()) != null)
				{
					if (msg.startsWith("[CONNECT]"))
					{
						...
						{	// 제대로 접속이 된경우
							...
							for (int i=0; i<bMan.size(); i++)						// 접속할 때마다 플레이어 이름을 저장
							{
								Player[i] = bMan.getHT(i).getUserName();
							}
							if(bMan.size() >= 2){
								for (int i=0; i<bMan.size()-1; i++) //////마지막에 접속한 플레이어의 이름이 중복된 닉네임인지 체크
								{
									if(Player[i].equals(Player[bMan.size()-1])){
										bMan.sendTo(bMan.size()-1, "중복된 닉네임입니다");
										bMan.sendTo(bMan.size()-1, "다른 닉네임을 입력해 주세요.");
										bMan.sendTo(bMan.size()-1, "재접속");
					...
					{
						ready = true;
						bMan.sendToAll(userName + "님 레디완료!");
						if (bMan.isReady())
						{
					...
					else if (msg.startsWith("[TURN]"))				// 클라이언트에서 카드뒤집기를 했을 경우
					{
						...
						if (Player[NowPlayer].equals(msg.substring(6)))	// 자기차례여부검사
						{
							...
							if (ClientCardCount[NowPlayer] == 0)
							{	// 클라이언트가 카드가 한장도 남지 않은 경우 죽음으로 처리
								dead[NowPlayer] = true;
								bMan.sendToAll("[UPDATEDEAD]" + Player[NowPlayer]);
								writer.println("[DEAD]" + Player[NowPlayer]);
								if (isEndGame() != -1) ///////////////////////////////////// 0 -> -1로 수정
								{
							...
							{	// 그외는 클라이언트에게 카드 다시그림 요청
								bMan.sendToAll("[REPAINT]" + Player[NowPlayer] + "|" + TurnCard[NowPlayer][TurnCardCount[NowPlayer]-1]);
								UpdateCardNum();
								NextPlayer();
								bMan.sendToAll(Player[NowPlayer] + "님 차례입니다.");
							}
							...
							for (int i=0; i<4; i++)
							{
								...
								{	// 종치기성공시 깔린 카드를 다 가져간다.
									...
									for (int j=0; j<4; j++)
									{
										if (!dead[j])
										{
											bMan.sendTo(j, "[SUCCESS]" + userName);
										}
									}
									isBell = false;
									...
									for (int m=ClientCardCount[a]; m>0; m--)	// 카드 섞기
									{
										int temp;
	...
	class BManager extends Vector
	{
		...
		void sendToOthers(HalliGalli_Thread ht, String msg)	// 자기를 제외한 유저에게 보내는 메시지
		{
			for (int i=0; i<size(); i++)
			{
				if (getHT(i) != ht)
